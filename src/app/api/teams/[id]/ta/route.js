...
export async function GET(request, { params }) {
  try {
    ...
    // for each assignment, get the ta name and email
    for (const assignment of assignments) {
      const ta = await db
        .collection("users")
        .findOne({ _id: new ObjectId(assignment.ta) });
      assignment.taName = ta ? ta.name : null;
      assignment.taEmail = ta ? ta.email : null;
    }
