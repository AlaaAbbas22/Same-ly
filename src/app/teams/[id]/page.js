  ...
  const fetchTeamDetails = async () => {
    try {
      ...
      if (response.ok) {
        setTeam(data);
        setIsEditor(
          data.editors.some((editor) => editor._id === session.user.id),
        );
        setIsStudent(
          data.students.some((student) => student._id === session.user.id),
        );
      }
    } catch (error) {
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between items-start lg:items-center gap-4 mb-6">
        {isEditor || isStudent ? (
          ...
          <h2 className="text-xl font-semibold">
            You are not a member of this team yet.
          </h2>
        )}
        <div className="flex flex-row flex-wrap gap-2 w-full lg:w-auto">
          ...
          {(isEditor || isStudent) && (
            <>
              <Link href={`${params.id}/myassignments`}>
                <Button variant="default">My Assignments</Button>
              </Link>
              {isEditor && (
                <Link href={`${params.id}/myta`}>
                  <Button variant="default">My Students' Assignments</Button>
                </Link>
              )}
            </>
          )}
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {isEditor && editor._id !== session.user.id && (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleRemoveMember(editor._id, "editor")
                          }
                        >
                          Remove
                        </Button>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        {isEditor && (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleRemoveMember(student._id, "student")
                            }
                          >
                            Remove
                          </Button>
